...
; % son las iteraciones para el aprendizaje
...
while m < epochs
    ...
    for i=1:M
        ...
        aj = bh + Wh * Xm;  %% ESCRIBIR EL CODIGO AQUI - #0: [-0.0530 0.3094]
        [y, dfh] = activation(aj);
        % Calcular ahora el valor de salida utilizando lo precedente
        ak	= bo + Wo*y; %% ESCRIBIR EL CODIGO AQUI - #0: 1.0133
        ...
        delta_j		=  transpose(dfh) .* Wo * delta_k ;%% ESCRIBIR EL CODIGO AQUI - #0: [0.3550 0.0388]
        % Ahora se actualizan los pesos
        %w_kj <- w_kj + eta*delta_k*y_j
        Wo	= Wo + eta * delta_k * transpose(y); %transpose(y) o zk; %% ESCRIBIR CODIGO AQUI
        %w_ji <- w_ji + eta*delta_j*Xm
        Wh	= Wh + eta * transpose(Xm .* delta_j);% y o Xm %% ESCRIBIR CODIGO AQUI
    end
