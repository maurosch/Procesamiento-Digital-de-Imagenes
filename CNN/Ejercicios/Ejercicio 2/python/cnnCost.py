...
def cnnCost(theta,images,labels,numClasses, filterDim,numFilters,poolDim,pred=False):
    ...
    imageDim = np.size(images,0); # height/width de imagen
    numImages = np.size(images,2); # numbero de imagenes
    ...
    Wc_grad = np.zeros(np.size(Wc));
    Wd_grad = np.zeros(np.size(Wd));
    bc_grad = np.zeros(np.size(bc));
    bd_grad = np.zeros(np.size(bd));
    ...
    # para Softmax layer
    activationsPooled = np.reshape(activationsPooled,[],numImages);
    ...
    ### IMPLEMENTACION AQUI ###
    hiddenSize = np.size(activationsPooled,1);
    ...
    return Wc_grad[:], Wd_grad[:], bc_grad[:], bd_grad[:]
